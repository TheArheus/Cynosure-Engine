...
class gpu_memory_heap
{
	...
	buffer*  AllocateBufferInternal(resource_descriptor& Desc, command_list* CommandList = nullptr);
	texture* AllocateTextureInternal(resource_descriptor& Desc, command_list* CommandList = nullptr);
...
public:
	...
	RENDERER_API resource_descriptor CreateTexture(const std::string& Name, void* Data, u32 Width, u32 Height, u32 Depth, const utils::texture::input_data& Info = {image_format::R8G8B8A8_UINT, image_type::Texture2D, image_flags::TF_Storage, 1, barrier_state::undefined, {border_color::black_opaque, sampler_address_mode::clamp_to_edge, sampler_reduction_mode::weighted_average, filter::linear, filter::linear, mipmap_mode::linear}});
	RENDERER_API resource_descriptor CreateTexture(const std::string& Name, u32 Width, u32 Height, u32 Depth, const utils::texture::input_data& Info = {image_format::R8G8B8A8_UINT, image_type::Texture2D, image_flags::TF_Storage, 1, barrier_state::undefined, {border_color::black_opaque, sampler_address_mode::clamp_to_edge, sampler_reduction_mode::weighted_average, filter::linear, filter::linear, mipmap_mode::linear}});

	[[nodiscard]] RENDERER_API resource_descriptor& GetResourceDescriptor(u64 ID);

	[[nodiscard]] RENDERER_API buffer* GetBuffer(resource_descriptor& Desc);
	[[nodiscard]] RENDERER_API buffer* GetBuffer(u64 ID);
	[[nodiscard]] RENDERER_API buffer* GetBuffer(command_list* CommandList, resource_descriptor& Desc);
	[[nodiscard]] RENDERER_API buffer* GetBuffer(command_list* CommandList, u64 ID);

	[[nodiscard]] RENDERER_API texture* GetTexture(resource_descriptor& Desc);
	[[nodiscard]] RENDERER_API texture* GetTexture(u64 ID);
	[[nodiscard]] RENDERER_API texture* GetTexture(command_list* CommandList, resource_descriptor& Desc);
