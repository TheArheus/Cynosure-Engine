...
layout(set = 0, binding = 5) uniform image3D VoxelTexture;
layout(set = 0, binding = 6) uniform image3D VoxelNormals;
...
layout(push_constant) uniform pushConstant { uint TriangleCount; };

void main()
{
	uint DataIdx = uint(gl_GlobalInvocationID.x);
	if(DataIdx >= TriangleCount) return;
